...
# Reuse existing nodebuilder-hope container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-hope; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-hope -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=ghcr.io/gethomepage/homepage:latest" \
    "${container}"
# Commit the image
